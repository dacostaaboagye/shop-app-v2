...
export class PlatformEventPipelinePublisher implements PlatformEventPublisher {
  constructor(
    private readonly dependencies: PlatformEventPipelineDependencies,
  ) {}
  ...
  async appendWithinTransaction(
    ...
    if (!this.dependencies.eventLogRepository.appendWithinTransaction) {
      throw new Error(
        "Platform event repository does not support transactional append.",
      );
    }

    await this.dependencies.eventLogRepository.appendWithinTransaction(
      event,
      db,
    );
  }
