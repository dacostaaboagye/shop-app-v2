...
/**
 * Like `useActiveLocationScope` but uses a `global-or-location` policy.
 * Pages that default to showing data across all assigned locations (e.g.
 * manager inboxes) use this variant. When no location is selected the context
 * kind is `"global"` and `accessibleLocationScopes` lists every eligible
 * location so the UI can render a filter selector.
 */
export function useActiveLocationScopeOptional(permission: string) {
  const pathname = usePathname();
  const router = useRouter();
  const searchParams = useSearchParams();
  const { isLoading, locationScopes } = useAuthorization();
  ...
  const operatingContext = useMemo(
    () =>
      resolveOperatingContext({
        activeLocationSlug,
        locationScopes,
        policy: { kind: "global-or-location", permission },
        urlLocationSlug,
      }),
    [activeLocationSlug, locationScopes, permission, urlLocationSlug],
  );
  ...
  useEffect(() => {
    if (isLoading || accessibleLocationScopes.length === 0) return;

    if (
      selectedLocationScope &&
      activeLocationSlug !== selectedLocationScope.locationSlug
    ) {
      setActiveLocationSlug(selectedLocationScope.locationSlug);
    }

    if (!selectedLocationScope) return;
    if (urlLocationSlug === selectedLocationScope.locationSlug) return;
