...
export function getTransferNotificationPresentation(
  ...
  switch (notification.eventType) {
    case "transfer.requested":
      return {
        detail: `${getNotificationActorLabel(notification)} requested ${formatQuantity(requested)}${route}.`,
        title: `Supply request ${reference} needs review`,
      };
    case "transfer.approved":
      return {
        detail: `${getNotificationActorLabel(notification)} approved ${formatQuantity(quantity)}${route}.`,
    ...
    case "transfer.dispatched":
      return {
        detail: `${getNotificationActorLabel(notification)} dispatched ${formatQuantity(quantity)}${route}${gtnReference ? ` under GTN ${gtnReference}` : ""}.`,
        title: `Supply request ${reference} is in transit`,
      };
    case "transfer.received":
      return {
        detail: `${getNotificationActorLabel(notification)} confirmed receipt${route}${gtnReference ? ` for GTN ${gtnReference}` : ""}.`,
...
export function getSupplierContactNotificationPresentation(
  notification: NotificationListItem,
) {
...
export function getSupplierProductNotificationPresentation(
  notification: NotificationListItem,
) {
  const supplierName = getPayloadString(notification, "supplierName");
...
export function getSupplierProcurementNotificationPresentation(
  notification: NotificationListItem,
) {
  const supplierName = getPayloadString(notification, "supplierName");
