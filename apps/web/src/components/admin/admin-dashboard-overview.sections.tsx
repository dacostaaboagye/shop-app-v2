...
export function AdminStoreProductivityPanel(props: {
  error: Error | null;
  isError: boolean;
  isPending: boolean;
  items: ReadonlyArray<{
  ...
  return (
    <Card className="border-border/70 bg-card shadow-none">
      <CardHeader>
        <CardTitle className="text-base">Shop productivity</CardTitle>
      </CardHeader>
      <CardContent>
        {props.isPending ? (
          ...
          <AppErrorBanner
            detail="Could not load location sales performance."
            error={props.error}
            title="Unable to load productivity"
          />
        ) : props.items.length ? (
          <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="flex min-w-0 flex-col gap-1">
                      <p className="type-data-value">{item.locationName}</p>
                      <p className="type-support text-xs">
                        Open location performance
                      </p>
                    </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="min-w-0 rounded-lg border border-border/50 bg-background/70 p-3">
                      ...
                      <p className="type-data-value mt-2 overflow-wrap-anywhere break-words">
                        {formatMoney(item.revenue, props.moneyProfile)}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <AppEmptyState
            description="Sales activity for visible shop locations will appear here."
            kind="no-data"
...
export function AdminDashboardTransferPanel(props: {
  ageingCount: number;
  ...
  return (
    <Card className="border-border/70 bg-card shadow-none">
      <CardHeader>
        <CardTitle className="text-base">Transfer attention</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        {props.isPending ? (
          ...
          <AppErrorBanner
            detail="Could not load transfer pressure."
            error={props.error}
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <TransferMetric
                label="Bottlenecks"
                value={props.bottleneckCount}
              />
              <TransferMetric label="Ageing" value={props.ageingCount} />
              <TransferMetric label="Exceptions" value={props.exceptionCount} />
            </div>
            ...
            {props.items.length ? (
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="flex min-w-0 flex-col gap-1">
                          ...
                          <p className="type-support text-xs">
                            {item.sourceLocationName} to {item.locationName}
                          </p>
                        </div>
              ...
              <AppEmptyState
                description="No open transfers need admin attention."
                kind="no-data"
...
function TransferMetric(props: { label: string; value: number }) {
  return (
    <div className="min-w-0 rounded-xl border border-border/60 bg-muted/25 p-4">
      <p className="text-xs font-medium leading-tight text-muted-foreground">
        {props.label}
      </p>
      <p className="type-data-value mt-2">{props.value}</p>
    </div>
  );
}
