...
import {
  ...
  PERMISSION_SKELETON_KEYS,
  replacePermissionsQuery,
} from "./permissions-page-client.support";
...
export function PermissionsPageClient() {
  ...
  const pageSize = PERMISSION_PAGE_SIZE_OPTIONS.includes(
    rawPageSize as (typeof PERMISSION_PAGE_SIZE_OPTIONS)[number],
  )
    ? rawPageSize
    : 25;
    ...
    <PageShell>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          value={permissionsQuery.data?.totalCount ?? 0}
        />
        <StatCard
      ...
      <AdminDirectoryFilterPanel
        ...
        searchId="permissions-filter-search"
        summary={`${formatCount(permissionsQuery.data?.totalCount ?? 0)} permissions${draftSearch ? " matching the current search" : " across the access catalogue"}`}
        value={draftSearch}
      />

      <AppTableWrapper>
        {permissionsQuery.isPending && !permissionsQuery.data ? (
          <StockWorkspaceTableSkeleton keys={PERMISSION_SKELETON_KEYS} />
        ) : (
          <>
            {permissionsQuery.isError ? (
              <div className="p-8">
                <AppErrorBanner
                  detail={getPermissionsErrorMessage(permissionsQuery.error)}
            ...
            <AppDataTable
              ...
              pagination={{
                ...
                page: safePage,
                pageSize,
                pageSizeOptions: PERMISSION_PAGE_SIZE_OPTIONS,
