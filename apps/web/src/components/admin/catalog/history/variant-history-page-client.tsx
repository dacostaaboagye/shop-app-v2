...
type VariantHistoryPageClientProps = {
  productSlug: string;
  variantSlug: string;
};

export function VariantHistoryPageClient({
  productSlug,
  variantSlug,
}: VariantHistoryPageClientProps) {
  ...
  if (!can("catalog.history.view")) {
    return (
      <CatalogHistoryPageView
        backHref={toRoute(`/admin/products/${productSlug}`)}
        backLabel="Back to product"
        description="You do not have permission to view change history for this variant."
  ...
  const variant =
    product?.variants.find((item) => item.slug === variantSlug) ?? null;
  const viewProps = buildVariantHistoryPageProps(product, variant, productSlug);

  if (!product || !variant) {
    return (
      <CatalogHistoryPageView {...viewProps}>
        <AppEmptyState
          description="The variant may have been removed, or the link is incorrect."
