...
import { SourceLocationField } from "@/components/worker/stock/supply-request-dialog-sections";
import { getAppErrorMessage } from "@/lib/errors/app-error";
import {
  fetchManagerSupplyRequestSources,
  postManagerSupplyRequestBatch,
  supplyRequestSourcesQueryKey,
} from "@/lib/react-query/stock-supply";

export function ManagerBulkSupplyRequestDialog({
  onOpenChange,
  open,
  targets,
}: {
  ...
  const sourceLocationsQuery = useQuery({
    ...
    queryFn: () => {
      ...
      return fetchManagerSupplyRequestSources(destination.locationId);
    },
  ...
  const mutation = useMutation({
    ...
    }) => postManagerSupplyRequestBatch(payload),
    onSuccess(data: BulkStockSupplyRequestResponse) {
      ...
      void Promise.all([
        ...
        queryClient.invalidateQueries({
          queryKey: ["supply-requests", "manager-location"],
        }),
        queryClient.invalidateQueries({
          queryKey: ["supply-requests", "manager-incoming"],
        }),
      ]);
      onOpenChange(false);
    },
  ...
  return (
    <AppDialog
      description="Request one or more items for the selected managed location in a single grouped supply request."
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label
                        ...
                        htmlFor={`manager-grouped-qty-${target.skuId}`}
                      >
                        Quantity
                      </label>
                      <Input
                        id={`manager-grouped-qty-${target.skuId}`}
                        inputMode="numeric"
                        min={1}
                        required
                        type="number"
